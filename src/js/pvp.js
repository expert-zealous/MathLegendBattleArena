  ...
  PVP.createRoom = function (fb, me, code, onFound, onFail, onState) {
    ...
    fs.setDoc(roomRef, _clean({
      ...
      unsub = fs.onSnapshot(roomRef, function (snap) {
        ...
        const d = snap.data();
        // Guest bisa terbaca lewat field guest/guestUid. Jangan mensyaratkan
        // keduanya sekaligus karena room lama/latensi bisa hanya membawa salah satu.
        if ((d.guestUid || d.guest) && d.status === 'waiting') {
          if (onState) onState('joining');
          const guestUid = d.guestUid || d.guest;
          const opp = { uid: guestUid, name: d.guestName || 'GUEST', hero: d.guestHero || 'raka', mr: d.guestMr || 1000, gear: d.guestGear || null };

          // Transisi WAITING -> PLAYING dilakukan atomik. Ini penting agar
          // listener host tidak hanya bergantung pada setDoc yang menimpa
          // seluruh dokumen dan berpotensi kalah oleh update guest.
          fs.runTransaction(db, async function (tx) {
            const latest = await tx.get(roomRef);
            if (!latest.exists()) throw new Error('ROOM_GONE');
            const x = latest.data();
            if (x.status === 'playing') return;
            if (x.status !== 'waiting') throw new Error('ROOM_NOT_WAITING');
            const gUid = x.guestUid || x.guest;
            if (!gUid) throw new Error('GUEST_MISSING');
            tx.update(roomRef, _clean({
              status: 'playing', host: uid, guest: gUid, guestUid: gUid,
              names: { host: me.name, guest: x.guestName || 'GUEST' },
              heroes: { host: me.hero, guest: x.guestHero || 'raka' },
              round: 0, q: null, ans: null, skillReq: false, surrender: false,
              seq: 0, frame: null, winner: null, result: null,
              hostAlive: Date.now(), guestAlive: Date.now(), updatedAt: Date.now()
            }));
            ...
            if (!done) {
              // Bila klien lain sudah lebih dulu mengubah status menjadi playing,
              // host tetap boleh masuk ke battle.
              fs.getDoc(roomRef).then(function (latest) {
                const x = latest.exists() ? latest.data() : null;
                if (x && x.status === 'playing' && (x.guestUid || x.guest)) {
                  const gUid = x.guestUid || x.guest;
                  finish({ role: 'host', roomId: code, opp: {
                    uid: gUid, name: x.guestName || opp.name, hero: x.guestHero || opp.hero,
                    mr: x.guestMr || opp.mr, gear: x.guestGear || opp.gear
                  }});
                } else {
                  done = true; stop(); onFail && onFail(String(e && (e.code || e.message) || e));
                }
              }).catch(function () {
                done = true; stop(); onFail && onFail(String(e && (e.code || e.message) || e));
              });
        ...
        } else if (d.status === 'playing' && (d.guestUid || d.guest) && !done) {
          // Recovery: jika host terlambat memasang listener/callback, jangan
          // menunggu guest lagi. Langsung masuk ke pertandingan aktif.
          const gUid = d.guestUid || d.guest;
          finish({ role: 'host', roomId: code, opp: {
            uid: gUid, name: d.guestName || 'GUEST', hero: d.guestHero || 'raka',
  ...
  PVP.joinRoom = function (fb, me, code, onFound, onFail, onState) {
    ...
    fs.runTransaction(db, async function (tx) {
      ...
      unsub = fs.onSnapshot(roomRef, function (snap) {
        if (done || !snap.exists()) return;
        const d = snap.data();
        if (d.status === 'playing' && (d.guest === uid || d.guestUid === uid)) {
